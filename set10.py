...
if __name__ == '__main__':
    ...
    while True:
        ...
        filename0 = os.path.join(BASE_DIR, 'python/vclog1.txt')
        f = open(filename0, "r")
        f1 = f.readlines()
        list_log = []
        for x in f1:
        ...
        val = ''
        ofile = os.path.join(BASE_DIR, 'python/results/'+ifile)
        with open(ofile, 'w+') as f2:
            writer = csv.writer(f2)
            writer.writerow(('ci:ruc:name:activity:status:salary:count',))
            for item in list2:
                # val = '{}:{}:{}:{}:{}:{}:{}'.format(item['ci'], item['ruc'], item['name'], item['activity'],item['status'], item['salary'], item['count'])
                val = str(item['ci'])+':'+str(item['ruc'])+':'+str(item['name'])+':'+str(item['activity'])+':'+str(item['status'])+':'+str(item['salary'])+':'+str(item['count'])
                # val = '%s:%s:%s:%s:%s:%s:%s'%(item['ci'], item['ruc'], item['name'], item['activity'],item['status'], item['salary'], item['count'])
                writer.writerow((val,))
        ...
        filelog = os.path.join(BASE_DIR, 'python/logs/'+logFile)
        f = open(filelog, 'w')
        f.write(str(message))
        f.close